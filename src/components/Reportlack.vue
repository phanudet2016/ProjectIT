<template>
  <div>
        ...
        <div class="row">
          ...
          <div class="card">
            <div class="card-block">
              <h4 class="card-title">
                รายงานข้อมูลการขาดแคลนอุปกรณ์ทางการแพทย์ ของแต่ละอุปกรณ์
    ...
    <div class="print" v-if="print === 'print'">
      ...
      <h1><b>รายงานข้อมูลสถิติการขาดแคลนอุปกรณ์</b></h1><br>
      ...
      <center>
      <h3 style="text-align:left;">รายงานข้อมูลการขาดแคลนอุปกรณ์แยกตามประเภท</h3>
      ...
      <!-- หาผลรวมการขาดแคลนแต่ละอุปกรณ์ -->
      <h3 style="text-align:left;">รายงานผลรวมการขาดแคลนของแต่อุปกรณ์</h3>
      <table border="1" style="margin-top: 10px;">
        <tr>
          <th style="width:500px;text-align: center;height:50px;">ชื่ออุปกรณ์</th>
          <th style="width:80px;text-align: center;">จำนวน</th>
        </tr>
      ...
      <h3 style="text-align:left;">รายงานข้อมูลการขาดแคลนอุปกรณ์ของแต่ละอุปกรณ์</h3>
      <table border="1" style="margin-top: 10px;">
        <tr>
          <th width="100px" style="text-align: center;">เลขที่การแจ้ง</th>
          <th width="150px" style="text-align: center;">วันที่แจ้ง</th>
          <th width="300px" style="text-align: center;">ชื่ออุปกรณ์</th>
          <th width="150px" style="text-align: center;">ประเภท</th>
          <th width="150px" style="text-align: center;">จำนวน</th>
          <th width="150px" style="text-align: center;">ชื่อ - สกุล ผู้แจ้ง</th>
          <th width="150px" style="text-align: center;">แผนก</th>
...
export default {
  ...
  computed: {
    searchEqm () {
      return this.requests.filter((requests) => {
        return requests.month.indexOf(this.search) > -1 && requests.year.indexOf(this.searchYear) > -1
      })
    },
    ggg () {
      this.support = 0
      ...
      for (let i = 0; i < this.searchEqm.length; i++) {
        if (this.searchEqm[i].category === 'วินิจฉัย') {
          this.diagnose = this.diagnose + this.searchEqm[i].amountEqm * 1
        }
        if (this.searchEqm[i].category === 'สนับสนุน') {
          this.support = this.support + this.searchEqm[i].amountEqm * 1
        }
        if (this.searchEqm[i].category === 'วินิจฉัยและรักษา') {
          this.diagnoseAndtreat = this.diagnoseAndtreat + this.searchEqm[i].amountEqm * 1
        }
        if (this.searchEqm[i].category === 'รักษา') {
          this.treat = this.treat + this.searchEqm[i].amountEqm * 1
        }
      }
      ...
      for (let i = 0; i < this.searchEqm.length; i++) {
        ...
        if (this.searchEqm[i].check === 0) {
          ...
          for (let p = 0; p < this.searchEqm.length; p++) {
            ...
            arraySum.push({
              nameEqm: this.searchEqm[i].nameEqm,
              amountEqm: this.sumAmount
            })
          }
          arrayLack.push({
            nameEqm: arraySum[arraySum.length - 1].nameEqm,
            amountEqm: arraySum[arraySum.length - 1].amountEqm
          })
        }
      }
      ...
      reportLackRef.child('/').set(arrayLack)
